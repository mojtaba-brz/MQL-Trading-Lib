...
//|                                                                  |
//+------------------------------------------------------------------+
string create_standard_comment(string pre_fix = "", string group_name = "", int group_index = -1, string desc = "")
{
...
//|                                                                  |
//+------------------------------------------------------------------+
bool standard_comment_to_order_order_specs(string order_comment, OrderSTDSpecs &order_std_specs)
{
    string comment_parts[];
    StringSplit(order_comment, ',', comment_parts);
    if(ArraySize(comment_parts) != 4) return false;
