...
{
public:    
    SessionsClass(string prefix_objects);
    ~SessionsClass() {ObjectsDeleteAll(0, _prefix_objects);};
    void on_tick_step();
private:    
    // Variables ----------------------------------------------------------------------------
    ...
    void draw_all_sessions();
    void draw_current_sessions();
};

SessionsClass::SessionsClass(string prefix_objects)
{
...
{
    MqlDateTime current_time_struct; TimeCurrent(current_time_struct);
    ...
    for(int i = 0; i<4; i++){
        ...
        if(in_session) {
            draw_session_on_the_current_chart_if_is_new(_prefix_objects, sessions[i].start_hour, sessions[i].end_hour, sessions[i].bg_clr, sessions[i].brdr_clr, sessions[i].name);
        } else {
...
void SessionsClass::draw_all_sessions()
{
