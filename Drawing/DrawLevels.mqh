...
void draw_major_and_minor_round_number_level_line_if_is_new(string prefix, int major_level_pips, int minor_level_pips, int num_of_major_levels_on_one_side = 3)
{
    ...
    calc_current_major_and_minor_levels(_Symbol, num_of_major_levels_on_one_side, major_level_pips, minor_level_pips, major_level_price_array, minor_level_price_array);
    ...
    for(int i = 0; i < ArraySize(major_level_price_array) - 1; i++) {
        for(int j = 1; j < 5; j++) {
            append_element(minor_level_price_array, major_level_price_array[i] + j * convert_pips_to_price(_Symbol, minor_level_pips));
        }
    }
}
